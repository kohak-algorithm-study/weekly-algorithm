...
이때 방문 판매원 A가 최종적으로 4번 회시에 가는 경로를 (1번 - 3번 - 5번 - 4번)으로 설정하면, 소개팅에도 참석할 수 있으면서 총 3만큼의 시간으로 이동할 수 있다.
...
dp = [[INF] * (N + 1) for _ in range(N + 1)]  # dp[a][b] == a번 노드에서 b번 노드까지의 최단 거리

for _ in range(M):
    a, b = map(int, input().split())
    dp[a][b], dp[b][a] = 1, 1  # 양방향으로 연결되어 있으므로

X, K = map(int, input().split())

if dp[1][K] == 1 and dp[K][X] == 1:
    print(2)
else:
