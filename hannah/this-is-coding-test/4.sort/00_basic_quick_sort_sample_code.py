...
# 위 과정을 반복한다.
# 그러다가 피벗값보다 큰 값과 피벗값보다 작은 값의 위치가 엇갈렸을 때는 (예를 들어 5, 4, 2, 0, 3, 1, 6, 9, chapter_7, 8인 상황) 작은 값(1)과 피벗값(5)의 위치를 바꿔준다.
# 그러면  4, 2, 0, 3, 1, 5, 6, 9, chapter_7, 8이 된다. 포인트는 5를 기준으로 5보다 앞에 있는 값들은 5보다 작은 수들이 모이게 되고, 5보다 뒤에 있는 값들은 5보다 큰 수들이 모인다.
# 여기까지 하면 "분활 완료" 상태다.
